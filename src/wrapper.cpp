...
// Hive constructor
Hive::Hive() : m_work_ptr(new boost::asio::io_service::work(m_io_service)), m_shutdown(0) {}
...
bool Hive::HasStopped() { return (boost::interprocess::ipcdetail::atomic_cas32(&m_shutdown, 1, 1) == 1); }
...
void Acceptor::HandleAccept(const boost::system::error_code &error, boost::shared_ptr<Connection> connection) {
  if (error || HasError() || m_hive->HasStopped()) {
    ...
    if (connection->GetSocket().is_open()) {
      ...
      if (OnAccept(connection, connection->GetSocket().remote_endpoint().address(),
...
boost::asio::bluetooth::bluetooth::acceptor &Acceptor::GetAcceptor() { return m_acceptor; }
...
void Acceptor::SetTimerInterval(int32_t timer_interval) { m_timer_interval = timer_interval; }

// Acceptor::HasError definition
bool Acceptor::HasError() { return (boost::interprocess::ipcdetail::atomic_cas32(&m_error_state, 1, 1) == 1); }
...
void Connection::StartSend() {
  if (!m_pending_sends.empty()) {
    boost::asio::async_write(m_socket, boost::asio::buffer(m_pending_sends.front()),
                             m_io_strand.wrap(boost::bind(&Connection::HandleSend, shared_from_this(),
...
void Connection::StartRecv(int32_t total_bytes) {
  if (total_bytes > 0) {
    ...
    boost::asio::async_read(m_socket, boost::asio::buffer(m_recv_buffer),
                            m_io_strand.wrap(boost::bind(&Connection::HandleRecv, shared_from_this(), _1, _2)));
  } else {
...
boost::asio::bluetooth::bluetooth::socket &Connection::GetSocket() { return m_socket; }
...
void Connection::SetReceiveBufferSize(int32_t size) { m_receive_buffer_size = size; }

// Connection::GetReceiveBufferSize definition
int32_t Connection::GetReceiveBufferSize() const { return m_receive_buffer_size; }
...
void Connection::SetTimerInterval(int32_t timer_interval) { m_timer_interval = timer_interval; }

// Connection::HasError definition
bool Connection::HasError() { return (boost::interprocess::ipcdetail::atomic_cas32(&m_error_state, 1, 1) == 1); }
