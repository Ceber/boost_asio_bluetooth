...
public:
  ...
  endpoint() : data_() {
    ...
    memset(&data_.bt.rc_bdaddr, 0, sizeof(data_.bt.rc_bdaddr));
  ...
  endpoint(uint8_t channel) : data_() {
    data_.bt.rc_family = AF_BLUETOOTH;
    bdaddr_t zero_addr = {0};
    data_.bt.rc_bdaddr = zero_addr;
  ...
  // Get the underlying endpoint in the native type
  const boost::asio::detail::socket_addr_type *data() const { return &data_.base; }

  // Get the underlying size of the endpoint in the native type
  std::size_t size() const { return sizeof(boost::asio::detail::sockaddr_bt_type); }
  ...
  std::size_t capacity() const { return sizeof(boost::asio::detail::sockaddr_bt_type); }
...
#include "asio_bluetooth/detail/impl/endpoint.ipp"
